...
class API1(threading.Thread):
    ...
    def __init__(self, host, localhost, port):
        ...
        # I am uncertain if this is meant to be the
        # local sqlite db in ../wera-app/app/db/app.db
        # or
        # the google developer console firebase credentials
        # security reasoning: the credentials are greyed out
        API1.localhost = localhost
        API1.user='{developer-console-user}'
        API1.password='{developer-console-password}'
        API1.host = '{backend-domain}' #host
        API1.database='app'

    def run(self):
        print('API1 #> '+self.host+':'+str(self.port))
        API1.app.run(host=self.host, port=self.port, debug=False)
